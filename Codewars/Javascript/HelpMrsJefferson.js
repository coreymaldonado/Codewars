// My first KYU6 solved without timing out for time complexity!
...
function shortestArrang(n) {
    // your code here
      function recursion(largestNumber){
        let newNumber = largestNumber
        let numbers = [newNumber]
        while ( numbers.reduce((a, c) => a + c, 0) < n ){
        ...
        if ( numbers.reduce((a, c) => a + c, 0) == n ){
          return numbers
        } else if ( numbers.reduce((a, c) => a + c, 0) > n ){
          console.log( numbers.reduce((a, c) => a + c, 0) )
          return recursion(largestNumber - 1)
        }
    }
    let largest = Math.ceil(n / 2)
    return recursion(largest);
  };